...
function bukaAbsenMassal() {
    ...
    document.getElementById('mAbsContent').innerHTML = D.k.filter(x => x.s === 'Aktif').map(x => {
        ...
        return `<div style="margin-bottom:12px; border-bottom:1px solid #f1f5f9; padding-bottom:8px;">
            ...
            <div class="form-row"><select id="ma-s-${x.id}" class="input"><option ${!exist||exist.s==='Hadir'?'selected':''}>Hadir</option><option ${exist&&exist.s==='Izin'?'selected':''}>Izin</option><option ${exist&&exist.s==='Sakit'?'selected':''}>Sakit</option></select>
            <input type="time" id="ma-j-${x.id}" class="input" value="${exist?exist.jm:D.s.b}"></div>
        </div>`;
    }).join(''); openModal('mAbs');
}
...
function simpanAbsenMassal() {
    ...
    D.k.filter(x => x.s === 'Aktif').forEach(x => {
        ...
        D.a.push({ id: Date.now() + Math.random(), k: x.id, tg: tgl, s: document.getElementById('ma-s-' + x.id).value, jm: document.getElementById('ma-j-' + x.id).value, jk: D.s.u, tp: 'full', ket: '' });
    }); saveData(); closeModal('mAbs'); rAbs();
}
...
function hitungGaji() {
    let id = parseInt(document.getElementById('gaji-kar').value), bln = document.getElementById('gaji-bulan').value; if(!id || !bln) return;
    let kr = D.k.find(x => x.id === id), hk = D.a.filter(x => x.k === id && x.tg.startsWith(bln) && x.s === 'Hadir').length;
...
window.onload = function() {
    ...
    if (level === "Mandor") {
        document.querySelectorAll(".nav-item").forEach(item => {
            ...
            if (txt.includes("karyawan") || txt.includes("gaji") || txt.includes("setting")) { item.style.style.display = "none"; }
        });
    }
    showPage('home');
};
